...
package body Wind_Turbine is
   ...
   procedure Read_Wind_Speed is 
      Speed: Integer;
   begin
      AS_Put_Line("Please enter the wind speed (mph) being read by the wind turbine sensors");
      loop 
   ...
   function Status_Tip_Brake_To_String(Status_Tip_Brake: Status_Tip_Brake_Type) return String is
   begin
      case Status_Tip_Brake is
          when Activated => return "Activated";
          when Deactivated => return "Deactivated";
      end case;
   end Status_Tip_Brake_To_String;
   
   -- Conver the mechanical brake status to a string
   function Status_Mechanical_Brake_To_String(Status_Mechanical_Brake: Status_Mechanical_Brake_Type) return String is
   begin
   ...
   begin
      ...
      AS_Put_Line("");
      AS_Put("Tip Brake = ");
      AS_Put_Line(Status_Tip_Brake_To_String(Status_Turbine.Status_Tip_Brake));
      AS_Put("Mechanical Brake = ");
      AS_Put_Line(Status_Mechanical_Brake_To_String(Status_Turbine.Status_Mechanical_Brake));
   end Print_Status;
   ...
   begin
      ...
      Status_Turbine := (Speed_Measured => 0,
                         Status_Tip_Brake => Activated,
